import {
    account,
    deployVoiSwap, getArc200Balance,
    getBalance, getClient,
    getSuggestedParams,
...
async function main() {
    ...
    const { success: successTransfer } = await contract.arc200_transfer(algosdk.getApplicationAddress(appId), BigInt(0), false, true);
    ...
    const bootstrapResult = await client.bootstrap(
        bootstrapArgs(),
        await getUnnamedResourcesAccessedFromMethod(
            client,
            'bootstrap',
            bootstrapArgs()
        )
    ...
    fs.writeFileSync(path.join(__dirname, './_deployed.ts'), `export const currentAppId = ${appId};\nexport const currentLptAssetId = ${lptToken};\n`);
