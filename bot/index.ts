...
import { calculateSwappableAmount } from './numbers';
...
// SLUOTPZ3QT4AWTV7GDVXPWU6CL2ST6FCCTU3JFTW64LPGG5GJU4GAQBMPM
...
class Bot {
    ...
    async getAlgoBalance() {
        return (await nodeClient.accountInformation(account.addr).do())['amount'];
    }
    ...
    async fetchOrders(arc200Id: bigint | number): Promise<Orderbook> {
        try {
            if (this.cachedOrders && ((Date.now() - this.cachedOrders.timestamp) < 3_000)) {
    ...
    async tick(arc200Id: number) {
        ...
        if (bestBuyOrder && bestSellOrder && bestBuyOrder.price >= bestSellOrder.price) {
            const commonAlgoAmount = Math.min(
                (await this.getAlgoBalance()) * 0.75,
            ...
            if (bestSellOrder && bestSellOrder.price < ammPrice) {
                const swapable = calculateSwappableAmount(ammInfo.algoBalance, ammInfo.arc200Balance, 6, arc200Token.decimals, bestSellOrder.price * 1.01);
                const commonAlgoAmount = Math.min((await this.getAlgoBalance()) * 0.75, Number(swapable), Number(bestSellOrder.algoAmount));

                if (commonAlgoAmount > MIN_ORDER_AMOUNT && swapable > 10 * MIN_ORDER_AMOUNT) {
                    console.log('amm to ob', commonAlgoAmount / 1e6, Number(bestSellOrder.algoAmount) / 1e6);
                    ...
                    console.log(commonAlgoAmount, bestSellOrder.price, Number(targetArc200Amount).toLocaleString('en'));
                ...
                const swapable = calculateSwappableAmount(ammInfo.arc200Balance, ammInfo.algoBalance, arc200Token.decimals, 6, (1 / bestBuyOrder.price) * 1.01);
                const commonAlgoAmount = Math.min((await this.getAlgoBalance()) * 0.75, Number(swapable), Number(bestBuyOrder.algoAmount));

                if (commonAlgoAmount > MIN_ORDER_AMOUNT && swapable > 10 * MIN_ORDER_AMOUNT) {
                    console.log('ob to amm', commonAlgoAmount / 1e6);
                    ...
                    const txns2 = await ammConnector.swapArc200ToVoi(
                        targetArc200Amount,
                        (BigInt(Math.floor(commonAlgoAmount)) * 98n) / 100n,
                    );
