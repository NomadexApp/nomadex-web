...
import type { DeflyWalletTransaction, SignerTransaction } from './util/model/deflyWalletModels';
import {
  base64ToUint8Array,
  composeTransaction,
  formatJsonRpcRequest
} from './util/transaction/transactionUtils';
import type { AlgorandChainIDs } from './util/deflyWalletTypes';
...
class WalletConnector {
  ...
  constructor(options?: DeflyWalletConnectOptions) {
    ...
    this.connector = null;
    this.shouldShowSignTxnToast = false;
  ...
  async signTransaction(
    ...
    if (this.platform === 'mobile') {
      ...
      if (!this.connector) {
        throw new Error('WalletConnector was not initialized correctly.');
      }
    }
