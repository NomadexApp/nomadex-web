...
export async function deployApp(deployment, algod, indexer) {
	...
	if (!existingDeployments && !indexer) {
		throw new Error(
			`Didn't receive an indexer client, but also didn't receive an existingDeployments cache - one of them must be provided`
		);
	}
	...
	const isSchemaBreak =
		isSchemaIsBroken(existingGlobalSchema, newGlobalSchema) || isSchemaIsBroken(existingLocalSchema, newLocalSchema);
	const replace = async () => {
