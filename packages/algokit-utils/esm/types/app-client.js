...
export class ApplicationClient {
	...
	async deploy(deploy) {
		...
		if (!this._creator) {
			throw new Error(
				"Attempt to `deploy` a contract without specifying `resolveBy: 'creatorAndName'` in the constructor"
			);
		}
	...
	async call(call) {
		if (
			...
			return {
				...
				return:
					result.methodResults?.length ?? 0 > 0 ? result.methodResults[result.methodResults.length - 1] : undefined,
			};
		}
		...
		if (args.method) {
			...
			return {
				...
				methodArgs: await Promise.all(
					args.methodArgs.map(async (arg, index) => {
						...
						switch (defaultValueStrategy.source) {
							...
							case 'global-state': {
								const state =
									defaultValueStrategy.source === 'global-state'
										? await this.getGlobalState()
										: await this.getLocalState(sender);
