...
export class OnChainStateWatcher {
	...
	getAccountWatcher(address: string) {
		const newStore = writable<AccountState>({
			address,
			amount: 0,
			asaBalances: {},
			lastUpdateAt: 0,
		});
		if (!address) return newStore;
		const store =
...
export function watchArc200Balance(
	...
	const update = async () => {
		try {
			const balance = await MySmartAsset.from(appId, nodeClient).arc200BalanceOf(address);
...
export function watchPoolTotalSupply(appId: number, duration = 15_000): ReturnType<typeof readable<bigint>> {
	...
	const update = async () => {
		try {
			const supply = await MySmartAsset.from(appId, nodeClient).arc200TotalSupply();
...
export async function getAccountBalance(address: string) {
	const accountState = <AccountState>await OnChainStateWatcher.getAccountState(address);
	return accountState.amount - (accountState['min-balance'] ?? 0);
}