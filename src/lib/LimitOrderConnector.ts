...
export class LimitOrders001ClientConnector extends LimitOrders001Client {
    ...
    async getUnnamedResourcesAccessedFromMethod<C extends LimitOrders001ClientConnector>(methodName: keyof ReturnType<C['compose']>, args: any = {}) {
    ...
    async createOrder(orderType: LimitOrderType, arc200Id: number, algoAmount: bigint, arc200Amount: bigint) {
        ...
        if (orderType === LimitOrderType.SELL_ALGO_FOR_ARC200) {
            ...
            await this.createAlgoSellOrder(
                args(),
                {
            ...
            const atc = await (await this.compose()).createAlgoBuyOrder(
                args(),
                {
                    accounts: [...resources.accounts],
                    boxes: [
