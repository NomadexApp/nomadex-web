...
import { getUnnamedResourcesAccessed, nodeClient, nodeClientAllowsCompile } from './_shared';
...
export class PoolConnector extends PoolClient {
	appId = 0;
	arc200AssetId = 0;
	...
	static async createPool(arc200_token: number): Promise<PoolConnector> {
		const removeNot = addNotification('pending', `Txn in progress`);

		let appId: number | bigint | undefined = 0;
		try {
			const client = new PoolConnector(0, 0, getTransactionSignerAccount(), nodeClientAllowsCompile);
			...
			const connector = new PoolConnector(arc200_token, Number(appId));
	...
	async swapVoiToArc200(voiAmount: bigint, minViaAmount: bigint) {
		...
		const opts = await this.getUnnamedResourcesAccessedFromMethod('swapAlgoAlphaToBeta', swapArgs(), optinTxns);
		const atc = await composer
			.swapAlgoAlphaToBeta(swapArgs(), {
				...opts,
				boxes: [
					...opts.boxes
				],
	...
	async swapArc200ToVoi(arc200Amount: bigint, minVoiAmount: bigint) {
		...
		const opts = await this.getUnnamedResourcesAccessedFromMethod('swapSmartAlphaToBeta', swapArgs(), approveTxns);

		const atc = await composer
			.swapSmartAlphaToBeta(swapArgs(), {
				...opts,
				apps: [...new Set([...opts.apps, this.arc200AssetId])],
			})
			.atc();
	...
	async mint(algoAmount: bigint, arc200Amount: bigint) {
		...
		const opts = await this.getUnnamedResourcesAccessedFromMethod('addAlgoSmartLiquidity', mintArgs(), approveTxns);

		const mintTxns = (
			await composer
				.addAlgoSmartLiquidity(mintArgs(), {
					...opts,
					apps: [...new Set([...opts.apps, this.arc200AssetId])],
				})
				.atc()
