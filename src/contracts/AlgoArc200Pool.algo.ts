...
export class AlgoArc200Pool extends Contract {
   ...
   * @returns 
   */
  private tokens_to_mint(issued: uint64, supply_of_a: uint64, supply_of_b: uint64, amount_of_a: uint64, amount_of_b: uint64): uint64 {
   ...
   * @returns 
   */
  private compute_token_return_amount_for_burning_lpt(lpt_issued: uint64, supply_of_token_in_app: uint64, lpt_amount: uint64): uint64 {
  ...
  burn(lpt_xfer_txn: AssetTransferTxn): void {
    ...
    const arc200_withdraw_amount = this.compute_token_return_amount_for_burning_lpt(
      lpt_issues, this.get_arc200_balance(), lpt_xfer_txn.assetAmount
    );
  ...
  swap_to_arc200(pay_txn: PayTxn, min_amount: uint64): uint64 {
    ...
    const admin_fee = this.compute_out_tokens_admin_fee(
      pay_txn.amount,
      balance - pay_txn.amount,
      arc200_balance,
    );

    const admin_fee_in_algos = this.compute_out_tokens(
      admin_fee,
      arc200_balance - admin_fee,
      balance,
      0
    );
