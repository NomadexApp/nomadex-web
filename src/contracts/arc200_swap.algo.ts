...
export class Arc200Swap extends Contract {
  ...
  private tokens_to_mint(issued: uint64, a_supply: uint64, b_supply: uint64, a_amount: uint64, b_amount: uint64): uint64 {
  ...
  burn(lpt_pay_txn: AssetTransferTxn): void {
    ...
    const arc200_withdraw_amount = this.compute_token_withdraw_amount(issued, this.get_arc200_balance(), lpt_pay_txn.assetAmount);
