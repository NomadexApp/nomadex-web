...
<PoolChartContext>
	{#snippet allevents(tokenA, tokenB, events)}
		<div class="br"></div>
				...
				<table class="table-auto w-full max-w-[900px] bg-[#00000033] backdrop-blur-[5px] rounded-[8px]">
					...
					<tbody>
						{#each someEvents as event}
							...
							{@const fromAmount = isSwapTxn
								? Number(event['fromAmount']) / (event['direction'] ? tokenB.unit : tokenA.unit)
								: 0}
							{@const toAmount = isSwapTxn
								? Number(event['toAmount']) / (event['direction'] ? tokenA.unit : tokenB.unit)
								: 0}
							{@const alphaAmount = isLiquidityTxn
								? Number(convertDecimals(event['amts'][0], tokenA.decimals, 6)) / 1e6
								: 0}
							{@const betaAmount = isLiquidityTxn
								? Number(convertDecimals(event['amts'][1], tokenB.decimals, 6)) / 1e6
								: 0}
							<tr>
