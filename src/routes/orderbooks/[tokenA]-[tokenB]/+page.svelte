...
<section class="">
	<div class="flex flex-col justify-start w-full flex-grow">
		<PoolChartContext bind:price />
	</div>
	<div class="flex flex-col justify-start pt-6 flex-grow max-w-[400px]">
		...
		{:else}
			{#each Object.keys(sortedLimitOrders).filter((key) => arc200Token.id === Number(key)) as key}
				<div class="w-full flex flex-col p-2 px-4 bg-base-200 relative">
					<div
						class="pool rounded-btn flex flex-col gap-2 min-w-[100px] sm:min-w-[300px] w-full max-w-[800px] text-[0.75rem]"
					>
						<div class="flex justify-evenly cursor-pointer select-none font-bold" on:click={() => {}} on:keydown>
							<span class="name mb-0 flex justify-between items-center w-full">
								<span class="w-14 text-nowrap">Price ({tokenB.ticker})</span>
								<span class="w-14 text-nowrap">Amount ({tokenA.ticker})</span>
								<span class="w-14 text-nowrap text-right">Total ({tokenB.ticker})</span>
				...
				<!-- <div class="h-[2px]" /> -->
				<div class="flex flex-col text-sm max-h-[700px] bg-base-200">
					<div class="half flex-grow flex flex-col py-1 bg-base-200 justify-start max-h-[215px] overflow-y-scroll">
						{#each Array(Math.max(buyOrders.length, 7 - buyOrders.length))
							...
							{#if limitOrder}
								...
								/>
							{:else}
								<div class="w-full flex flex-col p-4 px-4 bg-base-200 relative" />
							{/if}
						{/each}
						<div use:scrollIntoView />
					</div>
					<div class="h-[1.5rem] bg-base-200 py-1 px-4 text-[0.85rem]">{Number(price.toFixed(10))}</div>
					<div class="half flex-grow flex flex-col py-1 bg-base-200 justify-start max-h-[215px] overflow-y-scroll">
